...
@click.option(
    "--max_hets", "-m", default=2.0, help="Max heterozygotes per Mb in a homozygous block"
)
@click.option(
    "--max_het_fraction",
    "-f",
...
@click.option("--shortest_block", "-s", default=100000, help="Shortest block")
@click.option(
...
@click.option("--individual", "-k", type=int, help="Index of individual in vcf/bcf, 0-based.")
@click.option(
    "--block_constant",
    "-s",
    default=100000,
...
@click.argument("vcf", type=click.Path(exists=True))
@click.option(
    "roh", "-r", type=click.File("r"), help="Bcftools roh style TSV file with CHR,POS,AZ,QUAL."
)
@click.option(
...
def annotate(vcf, roh, bed, v14, quality_threshold, flag_upd_at_fraction, output, verbose, select_sample):
    ...
    if roh and not bed and not v14:
        ...
        click.echo(
            """Cannot use both BED and ROH at once. Please apply
...
@click.option("--wig/--no-wig", default=True, help="Produce wig file.")
@click.option("--pointsize", "-p", type=int, default=8, help="Size of the points (pixels)")
@click.option(
    "--rho", "-r", type=click.File("r"), required=True, help="Input RHO file produced from rhocall"
)
@click.option(
    "--minsnv", "-m", type=int, default=2, help="Minimum number of snvs for each plotted bin"
)
@click.option(
    "--maxsnv", "-M", type=int, default=20, help="Maximum number of snvs for each plotted bin"
)
@click.option(
    "--minaf",
    type=float,
...
@click.option("--maxaf", type=float, default=0.9, help="Maximum allele frequency")
@click.option("--aftag", type=str, default="1000GAF", help="The allele frequency tag to use.")
@click.option(
    "--minqual",
    "-q",
    type=int,
...
@click.option("--window", "-w", type=int, default=10000, help="Window size(bases)")
@click.option("--rsid/--no-rsid", "-s", default=False, help="Skip variants not containing an rsid")
@click.option(
    "--filter/--no-filter",
    "-n",
    default=True,
...
def viz(
    ...
    binned_zygosity = generate_bins(
        vcf, window, filter, mnv, minqual, rsid, minaf, aftag, maxaf, minsnv
    )
    roh = extract_roh(rho)
    if wig:
