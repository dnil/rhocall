...
def run_rhocall(
    ...
    for var in proband_vcf:
        ...
        if not skip_variant(var):
            ...
            if current_chr != None and var.CHROM != current_chr:
                ...
                logger.debug("Leaving chromosome %s for %s." % (current_chr, var.CHROM))
                ...
                if check_heterozygote(var, proband_vcf, individual):
                    ...
                    if block_ends:
                        logger.debug(
                            "Leaving block with %d hets and block"
            ...
            if block_ends:
                if hom_block_start != None:
                    ...
                    logger.info(
                        "Block ends before one was started at %s %d." % (var.CHROM, var.start)
                    )
    ...
    if hom_block_end and hom_block_start:
        ...
        if block_ends:
            ...
            if valid_block:
                end_block(
                    current_chr, hom_block_start, hom_block_end, current_block_hets, block_homs
                )
