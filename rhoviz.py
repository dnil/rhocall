        ...
        if line[0] == "#":
            ...
            if "#CHROM" in line:
                for contig in contigs:
                    bins[contig] = numpy.zeros(
                        (int(math.ceil(contigs[contig] / float(args.window))), 2)
                    )
            continue
...
parser.add_argument(
    ...
    type=str,
    required=True,
    help="output directory, the files will be named dir/chr.png, one picture is printed per chromosome",
)
parser.add_argument(
    "--rho", "-r", type=str, required=True, help="Input RHO file produced from rhocall"
)
...
parser.add_argument(
    ...
    type=int,
    default=100,
    help="do not add snvs to a bin if there quality is less than this value",
)
parser.add_argument("--pointsize", "-p", type=int, default=8, help="Size of the points (pixels)")
parser.add_argument(
    "--rsid", "-s", action="store_true", help="Skip variants not containing an rsid"
)
parser.add_argument(
    "--nofilter",
    "-n",
